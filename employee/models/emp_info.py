...
from sqlalchemy.sql.sqltypes import CHAR, Date, VARCHAR
...
class DeptManager(mysql.BaseModel):
    """DeptManager Table Class."""
...
class Departments(mysql.BaseModel):
    ...
    children_4 = relationship('DeptManager', back_populates='parent_4')
...
def get_emp(emp_no):
    ...
    with mysql.db_read_session() as session:
        ...
        sql = 'SELECT * \
            FROM employees \
            WHERE emp_no = {employee_id};'.format(
            employee_id=emp_no
        )
...
def get_emp_salary(emp_no):
    ...
    with mysql.db_read_session() as session:
        ...
        sql = ' SELECT a.emp_no, a.first_name, a.last_name, b.salary \
            ...
            WHERE a.emp_no = {employee_id};'.format(
            employee_id=emp_no
        )
        emp_response = session.execute(sql)
        result = emp_response.fetchall()
...
def get_emp_dept(dept_name):
    ...
    with mysql.db_read_session() as session:
        data = dict(
            ...
            dept_id=[],
            dept_name=[])
        sql = 'SELECT DISTINCT e.emp_no, e.first_name, \
        e.last_name, d.dept_no, d.dept_name \
        FROM  employees e \
        LEFT JOIN departments d \
        ON e.emp_no = d.emp_no \
        GROUP BY d.dept_name \
        HAVING d.dept_name = "{dept_name}";'.format(
...
def get_salary_range(start, end):
    ...
    with mysql.db_read_session() as session:
        ...
        sql = 'SELECT a.emp_no, a.first_name, a.last_name, b.salary \
            ...
            WHERE salary BETWEEN {start} and {end};'.format(
            start=start, end=end
        )
        emp_response = session.execute(sql)
        result = emp_response.fetchall()
...
def get_manager_dept(manager_id, dept_no):
    ...
    with mysql.db_read_session() as session:
        data = dict(
            ...
            dept_id=[],
            dept_name=[])
        sql = ' SELECT DISTINCT a.first_name, a.last_name, b.manager_id , \
            b.dept_no, c.dept_name \
            FROM employees a \
            ...
            HAVING b.manager_id = {manager_id} and \
            b.dept_no = "{dept_no}";'.format(
            manager_id=manager_id, dept_no=str(dept_no)
        )
        emp_response = session.execute(sql)
        result = emp_response.fetchall()
        for results in result:
...
def get_leaves_employee(emp_no):
    ...
    with mysql.db_read_session() as session:
        ...
        sql = ' SELECT a.emp_no, a.first_name, a.last_name, b.leaves_taken, \
            b.leaves_left, b.leaves_without_pay \
            FROM employees AS a \
            LEFT JOIN leaves AS b ON a.emp_no = b.emp_no \
            WHERE a.emp_no = {emp_no};'.format(
            emp_no=emp_no
        )
        emp_response = session.execute(sql)
        result = emp_response.fetchall()
...
def update_unpaid_leaves(emp_no, unpaid_leave):
    ...
    with mysql.db_read_session() as session:
        sql = ' UPDATE leaves \
            ...
            WHERE emp_no = {emp_no};'.format(
            emp_no=emp_no, unpaid_leave=unpaid_leave
        )
        session.execute(sql)
        emp_response = session.commit()  # noqa: F841
...
def update_taken_leaves(emp_no, taken_leave):
    ...
    with mysql.db_read_session() as session:
        sql = ' UPDATE leaves \
            ...
            WHERE emp_no = {emp_no};'.format(
            emp_no=emp_no, taken_leave=taken_leave
        )
        session.execute(sql)
        emp_response = session.commit()  # noqa: F841
...
def update_left_leaves(emp_no, left_leave):
    ...
    with mysql.db_read_session() as session:
        sql = ' UPDATE leaves \
            ...
            WHERE emp_no = {emp_no};'.format(
            emp_no=emp_no, left_leave=left_leave
        )
        session.execute(sql)
        emp_response = session.commit()  # noqa: F841
...
def set_employee(
    ...
    with mysql.db_read_session() as session:
        ...
        sql2 = ' UPDATE salaries \
            SET salary = {salary}\
            WHERE emp_no = {emp_no};'.format(
            emp_no=emp_no, salary=salary
        )
        sql3 = 'UPDATE departments \
            SET dept_no = "{dept_no}"\
            WHERE emp_no ={emp_no};'.format(
            emp_no=emp_no, dept_no=str(dept_no)
        )
